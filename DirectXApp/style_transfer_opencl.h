...
namespace StyleTransfer {
    ...
    class CopyMakeBorder : public OCLKernel {
    public:
        CopyMakeBorder(OCLEnv* env);
        virtual ~CopyMakeBorder();
        virtual bool Create(cl_program program);
        virtual bool Run();

        bool SetArguments(ID3D11Texture2D* in_surf, ID3D11Texture2D* out_surf, int cols, int rows, int border);
    private:
        cl_kernel   m_kernelY;
        cl_kernel   m_kernelUV;

        std::vector<OCLKernelArg*> m_argsY;
        std::vector<OCLKernelArg*> m_argsUV;

        OCLKernelArgSharedSurface m_inSurfY;
        OCLKernelArgSharedSurface m_outSurfY;

        OCLKernelArgSharedSurface m_inSurfUV;
        OCLKernelArgSharedSurface m_outSurfUV;

        OCLKernelArgInt m_colsY;
        OCLKernelArgInt m_rowsY;
        OCLKernelArgInt m_borderY;

        OCLKernelArgInt m_colsUV;
        OCLKernelArgInt m_rowsUV;
        OCLKernelArgInt m_borderUV;
    ...
    class FmtConversion : public OCLKernel {
    public:
        FmtConversion(OCLEnv* env);
        virtual ~FmtConversion();
        virtual bool Create(cl_program program);
        virtual bool Run();

        bool SetArgumentsNV12toRGB(ID3D11Texture2D* in_nv12Surf, cl_mem out_rgbSurf, int cols, int rows);
        bool SetArgumentsRGBtoNV12(cl_mem in_rgbSurf, ID3D11Texture2D* out_nv12Surf, int cols, int rows);
    private:
        cl_kernel   m_kernelNV12toRGB;
        cl_kernel   m_kernelRGBtoNV12;

        std::vector<OCLKernelArg*> m_argsNV12toRGB;
        std::vector<OCLKernelArg*> m_argsRGBtoNV12;

        OCLKernelArgSharedSurface m_surfY;
        OCLKernelArgSharedSurface m_surfUV;

        OCLKernelArgSurface m_surfRGB;
